...
BssidNum=${num:-1}
SSID1=${ssid1:-tianfy_SSID1}
SSID2=${ssid2:-tianfy_SSID2}
SSID3=${ssid3:-tianfy_SSID3}
SSID4=${ssid4:-tianfy_SSID4}
SSID5=
SSID6=
SSID7=
SSID8=
...
EOF
#检查并创建WiFi驱动配置链接
#	if [ ! -f /etc/Wireless/RT2860/RT2860.dat ]; then
...
#	rmmod rt2860v2_ap 
	...
	grep -rn rt2860v2_ap /proc/modules > /dev/null 2>&1
	if [ $? == 0 ]; then
		echo "rt2860v2 is exits!"
	else
		insmod rt2860v2_ap
		ifconfig ra0 up
	fi
...
#传参过来的第一个参数是Radio0
	...
	for vif in $vifs; do
		local ifname encryption key ssid mode crypto defaultkeyid cryptowpa2pskmixed
		...
		config_get crypto $vif crypto
		config_get cryptowpa2pskmixed $vif cryptowpa2pskmixed
		config_get defaultkeyid $vif defaultkeyid
		#是否隔离客户端
			...
			case "$encryption" in
				...
				wpa*|psk*|WPA*|Mixed|mixed)
					...
					*tkip*) 
					...
					echo "$crypto1" >>/tmp/wifi_encryption_${ifname}.dat
					iwpriv $ifname set AuthMode=$enc
					iwpriv $ifname set EncrypType=$crypto1
				...
				WEP|wep|wep-open|wep-shared)
					...
					iwpriv $ifname set DefaultKeyID=$defaultkeyid
					iwpriv $ifname set "SSID=${ssid}"
					echo 
					iwpriv $ifname set WscConfMode=0
					;;
...
config wifi-iface
	...
	option network	lan
	option mode     ap
	option ssid		tianfy_7620a
	option encryption psk+psk2
	option cryptowpa2pskmixed tkip+aes
	option key 88888888
EOF
