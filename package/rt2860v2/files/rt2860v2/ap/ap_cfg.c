...
/*add by duxt*/
INT	Set_ApCli_Wisp_Pid_Proc(IN PRTMP_ADAPTER	pAd, IN	PSTRING	arg);
...
/*add by duxt 	{"ApCliWispPid",				Set_ApCli_Wisp_Pid_Proc},*/
#ifdef APCLI_SUPPORT
	...
	{"ApCliWispPid",				Set_ApCli_Wisp_Pid_Proc},
#ifdef APCLI_AUTO_CONNECT_SUPPORT	
...
/* del by tianfy
		case OID_802_11_COEXISTENCE:
		     ...
		     break;
*/
/*modify by tianfy, add #ifdef DOT11N_DRAFT3,*/
#ifdef DOT11N_DRAFT3
		case OID_802_11_COEXISTENCE:
	            wrq->u.data.length = sizeof(BOOLEAN);
	            Status = copy_to_user(wrq->u.data.pointer, &pAd->CommonCfg.bBssCoexEnable, wrq->u.data.length);
	...
	IN	PSTRING			arg)
{
	...
	char temp[1024]={0};
	/*end of get wireless clients list from proc file to user by tianfy 2014-12-12*/
#ifdef CONFIG_RA_STAINFO //add by tianfy
	printk("\n");
#endif
	...
	for (i=0; i < MaxWcidNum; i++)
	{
			...
			&& (pEntry->Sst == SST_ASSOC))
		{
			...
			if(i == 0)
			{
				sprintf(temp,"%02x:%02x:%02x:%02x:%02x:%02x %d %d %d %s %d %d %d\n",
					pEntry->Addr[0], pEntry->Addr[1], pEntry->Addr[2],
					pEntry->Addr[3], pEntry->Addr[4], pEntry->Addr[5],
					(int)pEntry->Aid, (int)pEntry->PsMode, (int)pEntry->MmpsMode,
					GetBW(pEntry->HTPhyMode.field.BW), pEntry->HTPhyMode.field.MCS,
					pEntry->HTPhyMode.field.ShortGI, pEntry->HTPhyMode.field.STBC);
			}
			else
			{
				sprintf(temp,"%s %02x:%02x:%02x:%02x:%02x:%02x %d %d %d %s %d %d %d\n",
					temp,
					pEntry->Addr[0], pEntry->Addr[1], pEntry->Addr[2],
					pEntry->Addr[3], pEntry->Addr[4], pEntry->Addr[5],
					(int)pEntry->Aid, (int)pEntry->PsMode, (int)pEntry->MmpsMode,
...
/*add by duxt*/
INT	Set_ApCli_Wisp_Pid_Proc(
	IN	PRTMP_ADAPTER	pAd, 
	IN	PSTRING			arg)
{
    if(simple_strtol(arg, 0, 10) >= 0)
        g_wisp_pid = simple_strtol(arg, 0, 10);

	DBGPRINT(RT_DEBUG_TRACE, ("<== Set_Wisp_Pid_Proc(wisp_pid = %ld)\n", g_wisp_pid));
